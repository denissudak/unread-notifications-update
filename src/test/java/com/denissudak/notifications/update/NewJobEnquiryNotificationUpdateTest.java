...
public class NewJobEnquiryNotificationUpdateTest {

    @Mock
    private Notification notification;

    @Mock
    private NewJobEnquiryData thisNotificationNewJobEnquiryData;

    private NewJobEnquiryNotificationUpdate update;
    ...
    public void setUp() {
        ...
        when(notification.getNewJobEnquiryData()).thenReturn(thisNotificationNewJobEnquiryData);
        when(thisNotificationNewJobEnquiryData.getJobId()).thenReturn(jobId);

        update = new NewJobEnquiryNotificationUpdate(notification);
    }

    @Test
    public void shouldProcessJobDetailsChangeData() {
        // given
        ...
        // and
        InOrder inOrder = Mockito.inOrder(thisNotificationNewJobEnquiryData, notification);
        inOrder.verify(thisNotificationNewJobEnquiryData).setJobDetails(after);
        inOrder.verify(thisNotificationNewJobEnquiryData).update(notification);
    ...
    @Test
    public void shouldProcessNewJobEnquiry() {
        // when and then
        assertThat(update.process(new NewJobEnquiryData(jobId, mock(JobDetails.class)))).isTrue();
